...
def get_model():
    ...
    model = build_model_from_rootfile('/uscms_data/d2/dsperka/8TeV/MakeTBntuples/CMSSW_5_3_7/src/BDT/RootFiles_ForBDTLimits/comb_BDT_wp1300R_Histos-ge1btags.root',include_mc_uncertainties=True)
    ...
    model.set_signal_processes('wp1300R')
    ...
    for p in model.processes:
        if p == 'tops': 
            model.add_lognormal_uncertainty('scaled_rate',  math.log(1.15), p)
        if p == 'topsntb': 
            model.add_lognormal_uncertainty('scaled_rate',  math.log(1.15), p)

    #model.add_lognormal_uncertainty('wjets_rate',  math.log(1.30), 'wjets')
    model.add_lognormal_uncertainty('ttbar_rate',  math.log(1.15), 'ttbar')
    model.add_lognormal_uncertainty('elepeff', math.log(1.03), '*','elec_BDT_ge1btags')
    model.add_lognormal_uncertainty('ehlteff', math.log(1.03), '*','elec_BDT_ge1btags')
    model.add_lognormal_uncertainty('mulepeff', math.log(1.03), '*','mu_BDT_ge1btags')
    model.add_lognormal_uncertainty('muhlteff', math.log(1.03), '*','mu_BDT_ge1btags')
...
plot_exp.write_txt('wp1300R_comb_bayesian_limits_expected.txt')
plot_obs.write_txt('wp1300R_comb_bayesian_limits_observed.txt')
...
report.write_html('htmlout_wp1300R_comb')
