                   ...
                   Bool_t greyScale = kFALSE, Bool_t useTMVAStyle = kTRUE )
{
   ...
   for (Int_t ic=0; ic<ncls; ic++) {
      ...
      gStyle->SetPaintTextFormat(0);

      //h2->SetMarkerSize( 1.5 );
      h2->SetMarkerSize( 1.0 );// this will change the text size as well. 
      h2->SetMarkerColor( 0 );
      ...
      h2->SetLabelOffset( -0.0 );// label offset on x axis    
      h2->GetYaxis()->SetLabelOffset( -0.0);// label offset on x axis    
      ...
      // add comment    
      TText* t = new TText( 0.53, 0.90, "Linear correlation coefficients in %" );
      t->SetNDC();
      t->SetTextSize( 0.026 );
      t->AppendPad();    
      ...
      TString fp = fin.ReplaceAll(".root","");
      fp = fin.ReplaceAll("/","_");
      TString fname = "plots/";
      fname += hName[ic];
      c->Print(fname + fp + ".gif");
      c->Print(fname + fp + ".root");
      c->Print(fname + fp + ".C");
