...
void plot_efficiencies( TFile* file, TString fp, Int_t type = 2, TDirectory* BinDir)
{
   ...
   //TString name = histoname+ "_WithSignal.eps" ;
    //TString name = "SNN_output_"+ ntag +"_" + njet + ".gif" ;
...
void efficiencies( TString fin = "TMVA.root", Int_t type = 2, Bool_t useTMVAStyle = kTRUE )
{
   ...
   while ((key = (TKey*)nextDir())) {
      ...
      if (path.Contains("multicutMVA")){
         multiMVA=kTRUE;
	 plot_efficiencies( file,fp, type, d );
      }
   }
   plot_efficiencies( file, fp, type, gDirectory );
